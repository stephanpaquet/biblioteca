...
const filteredBooks = computed(() => {
  ...
  if (selectedCategory.value) {
    filtered = filtered.filter(
      (book) => book.categories && book.categories.includes(selectedCategory.value)
    );
  }

  // Apply status filter
  if (selectedStatus.value) {
    filtered = filtered.filter((book) => book.pivot?.status === selectedStatus.value);
  }
...
<template>
  <Layout>
    <div class="container mx-auto px-4 py-8">
      ...
      <div v-if="libraryStore.books.length === 0" class="text-center py-16">
        <div class="max-w-md mx-auto">
          ...
          <p class="text-gray-600 mb-6">
            Start building your personal library by adding books from search results.
          </p>
          <Button href="/search" variant="primary" left-icon="search">
      ...
      <div
        v-if="libraryStore.books.length > 0 && filteredBooks.length === 0"
      ...
      >
        <div class="max-w-md mx-auto">
          ...
          <Button variant="secondary" left-icon="clear" @click="clearFilters">
            Clear All Filters
          </Button>
        </div>
      </div>
