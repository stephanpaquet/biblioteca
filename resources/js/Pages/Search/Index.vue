...
<template>
  <Layout>
    <div class="container mx-auto px-4 py-8">
      <!-- Search Form -->
      <div class="mb-8">
        <SearchForm :initial-query="query" :initial-type="searchType" />
      </div>
        ...
        <div class="text-sm text-gray-600 space-y-1">
          <p>
            ...
            <span class="ml-2 text-blue-600"
              >({{ searchTypeLabels[searchType] || searchType }})</span
            >
          </p>
          <div v-if="Object.keys(filters).length > 1" class="flex flex-wrap gap-2 mt-2">
            ...
            <span
              v-if="filters.publishedAfter"
              class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded"
            >
              After: {{ filters.publishedAfter }}
            </span>
            <span
              v-if="filters.publishedBefore"
              class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded"
            >
              Before: {{ filters.publishedBefore }}
            </span>
            <span
              v-if="filters.printType"
              class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded"
            >
